...
export class ExperienceTicketingAndBookmarkFk1786518147139 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "experiences" ADD "ticketingLink" character varying`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "bookmarks" ALTER COLUMN "experienceId" TYPE uuid USING "experienceId"::uuid`,
    );
    await queryRunner.query(`
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "bookmarks" DROP CONSTRAINT "FK_bookmarks_experienceId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "bookmarks" ALTER COLUMN "experienceId" TYPE character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "experiences" ADD "category" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "experiences" DROP COLUMN "ticketingLink"`,
    );
  }
}
